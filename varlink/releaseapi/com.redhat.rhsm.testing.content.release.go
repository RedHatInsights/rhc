...
type UnsetReleaseIn struct {
	Metadata *Metadata `json:"metadata,omitempty"`
}
type UnsetReleaseOut struct {
	Success bool `json:"success"`
}
...
func (c Client) UnsetRelease(in *UnsetReleaseIn) (*UnsetReleaseOut, error) {
	if in == nil {
		in = new(UnsetReleaseIn)
	}
	out := new(UnsetReleaseOut)
	err := c.Client.Do("com.redhat.rhsm.testing.content.release.UnsetRelease", in, out)
	return out, unmarshalError(err)
}
...
type Backend interface {
	...
	UnsetRelease(*UnsetReleaseIn) (*UnsetReleaseOut, error)
}
...
func (h Handler) HandleVarlink(call *govarlink.ServerCall, req *govarlink.ServerRequest) error {
	...
	case "com.redhat.rhsm.testing.content.release.UnsetRelease":
		in := new(UnsetReleaseIn)
		if err := json.Unmarshal(req.Parameters, in); err != nil {
			return err
		}
		out, err = h.Backend.UnsetRelease(in)
	default:
