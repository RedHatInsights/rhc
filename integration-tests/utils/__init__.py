...
def configure_proxy(test_config, auth_proxy=False):
    ...
    try:
        ...
        print(f"Error during proxy configuration: {e}")
        return False
...
def _configure_yggdrasil_systemd_proxy_service(proxy_url, service_name):
    ...
    # Store proxy secret in secure location (0600 permissions)
    ...
    # Create systemd override with LoadCredential (no secrets in plain text)
...
"""
    ...
    print(f"Systemd proxy configuration updated for {service_name} service")
