...
def test_connect(external_candlepin, rhc, test_config, auth, output_format):
    ...
    if output_format is None:
        ...
        pass
        # TODO: parse result.stdout, when CCT-1155 is fixed. When --format is used, then
        #       not only "rhc connect" prints JSON document to stderr (not stdout)
    ...
    if pytest.service_name == "rhcd":
        if output_format is None:
            ...
            pass
            # TODO: parse result.stdout, when CCT-1155 is fixed
    else:
        if output_format is None:
