...
rhc-configure - Configure Red Hat connector features before or after registration
...
**content**
: Access to package repositories. This is the base feature that provides system identity with Red Hat and enables content access.
...
**enable** FEATURE
: Enable the specified feature. This sets the user preference and, if the system is already registered, activates the feature on the system. Dependencies are automatically enabled.

**disable** FEATURE
: Disable the specified feature. This sets the user preference and, if the system is already registered, deactivates the feature on the system. Dependent features are automatically disabled.
...
Before registration, the STATE column shows x for all features since no services are running yet. After registration, STATE reflects whether the underlying services (subscription-manager, insights-client, yggdrasil) are actually enabled and running.
...
remote-management  ✓       x      Red Hat Ligthspeed remote management
...
After the system is registered, you can change features dynamically:
...
remote-management  ✓       ✓      Red Hat Ligthspeed remote management
...
```
[...]
features = { "analytics" = true, "remote-management" = false }
```
...
The command cannot change the server a system is registered to (ConsoleDot vs Satellite) without re-registration. Use **rhc disconnect** and **rhc connect** for that purpose.
...
**Enable remote management on a registered system:**
...
- Attempting to enable a feature without its dependencies
...
Red Hat Client Tools Team

# COPYRIGHT

Copyright 2024-2025 Red Hat, Inc. Licensed under the GNU General Public License v3.0 or later.