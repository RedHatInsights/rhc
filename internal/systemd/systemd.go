...
func NewConnectionContext(ctx context.Context, connectionType ConnectionType) (*Conn, error) {
	...
	if connectionType == ConnectionTypeSystem {
		...
		if err != nil {
			return nil, fmt.Errorf("cannot establish system connection to systemd: %v", err)
		}
	} else {
		conn, err = systemd.NewUserConnectionContext(ctx)
		if err != nil {
			return nil, fmt.Errorf("cannot establish user connection to systemd: %v", err)
		}
	}
...
func (c *Conn) StopUnit(name string, wait bool) error {
	...
	if wait {
		if err := c.waitForState(name, "inactive", 1*time.Second); err != nil {
