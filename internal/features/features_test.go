...
// TestGetFeaturesFromFiles_ValidTOML tests parsing valid TOML configurations
func TestGetFeaturesFromFiles(t *testing.T) {
	tests := []struct {
		description        string
		tomlContent        string
		want               conf.Features
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			confFeatures, err := GetFeaturesFromFiles(tmpFilePath)
			if err != nil {
...
func TestConsolidateSelectedFeatures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			err := ConsolidateSelectedFeatures(test.config, &test.input.enabledFeatures, &test.input.disabledFeatures)
			...
			if test.wantError != nil {
				...
				slices.Sort(test.input.enabledFeatures)
				slices.Sort(test.want.enabledFeatures)
				if !slices.Equal(test.input.enabledFeatures, test.want.enabledFeatures) {
					t.Errorf("enabled features mismatch: got %v, want %v", test.input.enabledFeatures, test.want.enabledFeatures)
				}

				slices.Sort(test.input.disabledFeatures)
				slices.Sort(test.want.disabledFeatures)
				if !slices.Equal(test.input.disabledFeatures, test.want.disabledFeatures) {
