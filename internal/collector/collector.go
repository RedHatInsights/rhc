...
func newConfig(id string, dto *configDto) (Config, error) {
	...
	if dto.Meta.Type != defaultMetaType {
	...
	if dto.Ingress.User == "" {
		dto.Ingress.User = defaultUser
	}
	if dto.Ingress.Group == "" {
		dto.Ingress.Group = defaultGroup
	}
	...
	if dto.Meta.Feature != "" && dto.Meta.Feature != "analytics" {
	...
	return Config{
		ID:                 id,
		Name:               dto.Meta.Name,
		IsAnalyticsFeature: dto.Meta.Feature == "" || dto.Meta.Feature == "analytics",
