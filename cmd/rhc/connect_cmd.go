...
func connectRHSM(ctx *cli.Context, connectResult *ConnectResult) {
	...
	if err != nil {
		...
		slog.Warn("Skipping generation of Red Hat repository file")
		ui.Printf(
...
func connectInsightsClient(connectResult *ConnectResult) {
	...
	slog.Info("Connecting to Red Hat Insights")
...
func connectAction(ctx *cli.Context) error {
	...
	for _, feature := range features.KnownFeatures {
		if feature.Enabled {
			...
			slog.Info(fmt.Sprintf("Feature '%s' enabled", feature.ID))
		} else {
			if ui.IsOutputMachineReadable() {
				switch feature.ID {
				case "content":
			...
			slog.Warn(fmt.Sprintf("Feature '%s' disabled", feature.ID))
		}
	}
